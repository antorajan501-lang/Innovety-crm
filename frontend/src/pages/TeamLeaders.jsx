...
const TeamLeaders = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries({ ...formData, role: 'TEAM_LEADER' }).forEach(([k, v]) => fd.append(k, v || ''));
            ...
            {users.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-6 py-10 text-center text-muted-foreground">
                  No Team Leaders registered matching search filter.
                </td>
              </tr>
            ) : (
              users.map((item) => (
              ...
              <div className="flex flex-col gap-2 border border-dashed border-border/60 rounded-xl p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    />
                    <label htmlFor="edit-tl-pic" className="cursor-pointer inline-flex items-center gap-1.5 rounded-lg border border-border bg-muted/40 px-3 py-1.5 text-xs font-semibold hover:bg-muted">
